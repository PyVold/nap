...
class AnalyticsService:
    ...
    @staticmethod
    async def _fetch_audit_results():
        """Fetch audit results from rule service"""
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(f"{AnalyticsService.RULE_SERVICE_URL}/audit/results")
    ...
    async def create_snapshot(db: Session, device_id: Optional[int] = None) -> ComplianceTrendDB:
        ...
        """
        # Get the most recent audit results from rule service
        all_results = await AnalyticsService._fetch_audit_results()
        
        # Filter by device if specified
        if device_id:
            results = [r for r in all_results if r.get('device_id') == device_id]
        else:
            # Get latest result per device for overall snapshot
            device_results = {}
            for result in all_results:
                dev_id = result.get('device_id')
                if dev_id not in device_results or result.get('timestamp', '') > device_results[dev_id].get('timestamp', ''):
                    device_results[dev_id] = result
        ...
        total_compliance = sum(r.get('compliance', 0) for r in results)
        overall_compliance = total_compliance / total_devices if total_devices > 0 else 0.0
        
        compliant_devices = sum(1 for r in results if r.get('compliance', 0) >= 90)
        failed_devices = sum(1 for r in results if r.get('compliance', 0) < 70)
    ...
    async def get_dashboard_summary(db: Session) -> Dict:
        ...
        # Devices at risk (compliance < 70%) - need to fetch from audit results
        all_results = await AnalyticsService._fetch_audit_results()
        device_results = {}
        for result in all_results:
            dev_id = result.get('device_id')
        ...
        devices_at_risk = sum(1 for r in device_results.values() if r.get('compliance', 0) < 70)
